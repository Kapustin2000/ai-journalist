<template>
  <div
    ...
    id="static-toolbar"
    class="flex bg-primary-foreground w-max h-full justify-center items-center rounded-md shadow-md p-3"
  >
    <!-- <div class="flex items-center relative">
      <button
        @click="toggleAIContentTypeMenu"
        :class="{ 'bg-gray-100/80 dark:text-gray-600': showAIContentTypeMenu }"
        class="flex items-center text-md px-2.5 py-1.5 rounded hover:bg-gray-100/80 dark:hover:text-gray-600"
      >
      ...
      <div
        ...
        class="w-max absolute top-10 start-0 bg-white dark:border dark:border-white dark:bg-gray-600 rounded-md shadow-md z-10 p-1.5 min-w-[140px]"
      >
        <button
          ...
          :class="{
            'bg-gray-100/80 rounded dark:text-gray-600': editor.isActive(
              type.name,
              type.attrs
            ),
          }"
          class="flex items-center justify-center w-full text-start px-3 py-2 hover:bg-gray-100/80 dark:hover:text-gray-600 rounded"
        >
          <div
            class="border border-gray-200 rounded-full p-1 grid place-items-center"
          >
    ...
    <div class="flex items-center relative">
      <button
        ...
        class="cursor-pointer flex items-center text-md px-2.5 py-1.5 rounded hover:bg-secondary transition duration-150"
      >
        {{ currentContentType }}
        <ChevronDown class="w-3.5 h-3.5 ms-1" />
      </button>
      <div
        ...
        v-on-click-outside="toggleContentTypeMenu"
        class="w-max absolute top-10 left-0 bg-primary-foreground border border-border rounded-md shadow-md z-10 p-1.5 min-w-[140px]"
      >
        <button
          ...
          class="flex items-center justify-center w-full text-start px-3 py-2 hover:bg-secondary transition duration-150 cursor-pointer rounded"
        >
          <div
    ...
    <div class="flex items-center space-x-2">
      <button
        ...
        :title="action.label"
        class="rounded hover:bg-secondary w-8 h-8 grid place-items-center"
      >
    ...
    <!-- <div class="flex items-center relative">
      <button
        ...
        class="ms-2 rounded hover:bg-secondary w-8 h-8 grid place-items-center cursor-pointer"
      ...
      <div
        v-if="showColorMenu"
        v-on-click-outside="toggleColorMenu"
      ...
      >
        <button
          ...
          :class="{
            'bg-secondary ': editor.isActive('textStyle', {
              color: color.value,
            }),
          }"
          class="flex items-center w-full text-start px-3 py-2 hover:bg-secondary rounded transition duration-150 cursor-pointer"
        >
          <span
            class="w-4 h-4 rounded-full me-2 border border-border"
...
import FormatColorText from '~/assets/svg/editor/FormatColorText.vue';
...
import UploadVideo from '~/assets/svg/editor/Video.vue';
import ImagePlus from '~/assets/svg/editor/ImagePlus.vue';
import type { Range } from '@tiptap/core';
import { vOnClickOutside } from '@vueuse/components';
import { PercentCircleIcon, PercentIcon } from 'lucide-vue-next';
...
const contentTypes: ContentType[] = [
  ...
  {
    name: 'image',
    label: t('editor.toolbar.content.image.title'),
    icon: ImagePlus,
  ...
  {
    name: 'video',
    label: t('editor.toolbar.content.video.title'),
    icon: UploadVideo,
    command: ({ editor, range }) => {
      editor
        .chain()
        .focus()
        .deleteRange(range)
        .insertContent('<custom-video></custom-video>')
        .run();
  ...
  },
  {
    ...
    command: ({ editor, range }) => {
      editor
        ...
        .focus()
        .deleteRange(range)
        .insertContent('<advertising-block></advertising-block>')
        .run();
...
const toggleContentTypeMenu = () => {
  ...
  console.log(showContentTypeMenu.value);
};
...
const setContentType = (
  ...
  command({ editor: props.editor!, range: props.editor!.state.selection });
  showContentTypeMenu.value = false;
};
